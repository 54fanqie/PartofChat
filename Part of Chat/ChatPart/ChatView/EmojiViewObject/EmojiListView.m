...
-(void)setEmojiViewFrame:(CGRect)emojiViewFrame{
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        self.frame=emojiViewFrame;
        [self initEmojiList:CGRectGetWidth(emojiViewFrame)];
    });
}
...
-(void)initEmojiList:(CGFloat)width{
    ...
    for (NSInteger i=0;i<self.typesEmoji.count;i++) {
        ...
        //获取本地表情包
        NSMutableArray * emojiArray = [NSMutableArray arrayWithArray:[ReadEmojiModel emojiInfoInWithBundleName:dic[@"bundle"] plistName:dic[@"plist"]]];
...
-(void)selectEmoji:(NSInteger)index{
    ...
    if (_delegate &&[_delegate respondsToSelector:@selector(insertEmoji:)]) {
        [_delegate insertEmoji:emojiInfo];
    }
